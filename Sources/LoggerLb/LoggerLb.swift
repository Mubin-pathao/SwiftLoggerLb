...
public struct StreamLogHandler: LogHandler {
    ...
    public func log(
        ...
        stream.write(
            "\(self.timestamp()) \(iconForLevel(logLevel: level)) \(level) \(self.label) :\(prettyMetadata.map { " \($0)" } ?? "") [\(source)] \(message) \(file), \(function) \(line)\n"
        )
    }
    
    private func iconForLevel(logLevel : Logger.Level) -> String {
        switch logLevel {
        case .trace:
            return "⬜️"
        case .debug:
